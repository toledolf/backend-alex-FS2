...
export class UsuarioCTRL{
        ...
        gravar(requisicao, resposta){
            ...
            if(requisicao.method === "POST" && requisicao.is('application/json')){
                ...
                if(cpf && nome && dataNasc && email && tel && sexo && cidade && uf && treinador && jogador){

                    const usuario = new Usuario(cpf, nome, dataNasc, email, tel, sexo, cidade, uf, treinador, jogador);
        ...
        excluir(requisicao, resposta){
            ...
            if(requisicao.method === "DELETE" && requisicao.is('application/json')){
                ...
                if(cpf){
                    ...
                    usuario.removerDoBancoDados().then(()=>{
                        resposta.status(200).json({
                            status: true,
        ...
        consultarPeloCPF(requisicao, resposta){
            ...
            if(requisicao.method === "GET"){
                const usuario = new Usuario();
                usuario.consultarCPF(cpf).then((usuarios)=>{
                    resposta.status(200).json(usuarios);
                }).catch((erro) => {
                    resposta.status(500).json({
                        status:false,
